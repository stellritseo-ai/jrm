...
export function FloatingChat() {
  ...
  useEffect(() => {
    ...
    const sessionId = localStorage.getItem("jrm-client-chat-session-id");
  ...
  const handlePreChatSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("jrm-client-chat-session-id", newSession.id);
  ...
  return (
    <div className="fixed bottom-4 right-3 sm:bottom-6 sm:right-6 z-50 flex flex-col items-end select-none font-sans">
            ...
            <form onSubmit={handlePreChatSubmit} className="flex-1 p-5 bg-[#fbfaf7] flex flex-col justify-center overflow-y-auto space-y-3.5 text-left">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#f4f6f8] hover:bg-[#ebedf0] focus:bg-white border-none rounded-xl py-2 px-3.5 text-xs text-neutral-800 placeholder-neutral-450 focus:outline-none focus:ring-2 focus:ring-[#4a6741]/20 transition-all"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setPreChatData(prev => ({ ...prev, city: e.target.value }))}
                  className="w-full bg-[#f4f6f8] hover:bg-[#ebedf0] focus:bg-white border-none rounded-xl py-2 px-3.5 text-xs text-neutral-800 placeholder-neutral-450 focus:outline-none focus:ring-2 focus:ring-[#4a6741]/20 transition-all cursor-pointer"
                >
              ...
              <div className="flex-1 overflow-y-auto p-4 bg-[#fbfaf7] space-y-4">
                {(chatSession?.messages || []).map((msg) => (
                  <div key={msg.id} className={`flex flex-col ${msg.sender === "client" ? "items-end" : "items-start"}`}>
                    <div
                      className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-xs md:text-sm leading-relaxed shadow-[0_2px_8px_rgba(0,0,0,0.02)] ${msg.sender === "client"
                    ...
                    {msg.sender === "admin" && msg.text.includes("remodeling or outdoor project") && (
      ...
      <button
        onClick={handleToggle}
        className={`flex h-14 w-14 items-center justify-center rounded-full transition-all duration-300 shadow-2xl relative group active:scale-95 cursor-pointer bg-white border-2 hover:bg-neutral-50 ${isOpen
          ? "border-[#2c241d] text-[#2c241d] rotate-90"
