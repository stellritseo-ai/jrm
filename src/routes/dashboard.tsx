...
function DashboardPage() {
  ...
  // Admin password reset helper states
  const [resettingUser, setResettingUser] = useState<PortalUser | null>(null);
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      const res = await updateUserCredentials(currentUser.id, updateUsername, updateCurrentPassword, updateNewPassword);
      if (res.success) {
  ...
  // Admin reset user password handler
  const handleAdminResetPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentUser || !resettingUser || !resetNewPassword.trim()) {
      toast.error("Password cannot be empty.");
      return;
    }

    const toastId = toast.loading(`Resetting password for '${resettingUser.username}'...`);
    try {
      const res = await adminResetUserPassword(currentUser.id, resettingUser.id, resetNewPassword);
      if (res.success) {
        toast.success(`Password for '${resettingUser.username}' has been updated successfully!`, { id: toastId });
  ...
  // Gallery filter & edit state
  const [galleryFilter, setGalleryFilter] = useState("all");
  const [editingPhoto, setEditingPhoto] = useState<GalleryPhoto | null>(null);
  const [editPhotoTitle, setEditPhotoTitle] = useState("");
  const [editPhotoCategory, setEditPhotoCategory] = useState("General");
  const [editPhotoCaption, setEditPhotoCaption] = useState("");
  const [isCustomCategory, setIsCustomCategory] = useState(false);
  const [customCategoryText, setCustomCategoryText] = useState("");
  const [selectedUploadFiles, setSelectedUploadFiles] = useState<File[]>([]);
  const [selectedUploadCategory, setSelectedUploadCategory] = useState("Palm Trees");
  ...
  useEffect(() => {
    ...
    // Connect to relay sidecar (dynamic URL based on environment)
    const relayUrl = (import.meta.env as any).VITE_CHAT_RELAY_URL || 
      (window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1" 
        ? "http://localhost:3001" 
        : "");
        
    if (!relayUrl) {
      console.warn("[Socket] Chat relay URL not set for production. Skipping WebSocket connection.");
      return;
    }
  ...
  useEffect(() => {
    getLeads()
      .then(setLeads)
      .catch((err) => console.error("[Dashboard] Error loading leads:", err));
      
    getReviews()
      .then(setReviews)
      .catch((err) => console.error("[Dashboard] Error loading reviews:", err));
    ...
    getChatSessions()
      ...
      .catch((err) => console.error("[Dashboard] Error loading chat sessions:", err));
    ...
    getWebEmails()
      ...
      .catch((err) => console.error("[Dashboard] Error loading web emails:", err));
    ...
    getGalleryPhotos()
      ...
      .catch((err) => console.error("[Dashboard] Error loading gallery photos:", err));

    getPortalSettings()
      .then((data) => {
        if (data) setSettings(data);
      })
      .catch((err) => console.error("[Dashboard] Error loading portal settings:", err));
  }, []);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      getChatSessions()
        .then((sessions) => setChatSessions(sortByLatest(sessions)))
        .catch((err) => console.error("[Dashboard Poll] Error syncing chat sessions:", err));
        
      getWebEmails()
        .then(setWebEmails)
        .catch((err) => console.error("[Dashboard Poll] Error syncing web emails:", err));
    }, 3000);
  ...
  const [editProjectType, setEditProjectType] = useState<string>("remodeling");
  ...
  const handleOpenLeadDetails = (lead: Lead) => {
    ...
    setEditProjectType(lead.projectType || "remodeling");
  ...
  const handleExecuteGalleryPhotoUpload = async () => {
    if (selectedUploadFiles.length === 0) {
      toast.error("Please choose at least one image to upload.");
      return;
    }

    const toastId = toast.loading(`Uploading ${selectedUploadFiles.length} photo(s)...`);
    setUploadProgress({ current: 0, total: selectedUploadFiles.length });
    ...
    try {
      ...
      for (let i = 0; i < selectedUploadFiles.length; i++) {
        ...
        setUploadProgress({ current: i + 1, total: selectedUploadFiles.length });
        const updated = await uploadGalleryPhoto(base64String, selectedUploadCategory);
      ...
      toast.success(`${selectedUploadFiles.length} photo(s) uploaded and categorized under "${selectedUploadCategory}" successfully!`, { id: toastId });
  ...
  const handleOpenEditPhotoDetails = (photo: GalleryPhoto) => {
    setEditingPhoto(photo);
    setEditPhotoTitle(photo.title || "");
    setEditPhotoCaption(photo.caption || "");
    
    const standardCategories = ["General", "Remodeling", "Hardscapes", "Outdoor Kitchens", "Outdoor Fireplaces", "Landscaping", "Artificial Turf", "Fencing", "New Construction"];
    const currentCategory = photo.category || "General";
    
    if (standardCategories.includes(currentCategory)) {
      setEditPhotoCategory(currentCategory);
      setIsCustomCategory(false);
      setCustomCategoryText("");
    } else {
      setEditPhotoCategory("Custom");
  ...
  const handleSavePhotoDetails = async (e: React.FormEvent) => {
    ...
    const finalCategory = isCustomCategory ? customCategoryText.trim() || "General" : editPhotoCategory;
    const toastId = toast.loading("Saving photo details...");
    
    try {
      const updated = await updateGalleryPhotoDetails(editingPhoto.id, {
      ...
      toast.success("Photo details updated successfully.", { id: toastId });
    } catch (err: any) {
      console.error(err);
      toast.error(err.message || "Failed to update details.", { id: toastId });
    }
  };
  ...
  const handleDeleteChatSession = async (sessionId: string, e: React.MouseEvent) => {
    ...
    const confirmed = window.confirm("Delete this chat conversation? This cannot be undone.");
    if (!confirmed) return;
    try {
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    if (isViewer) return;
    try {
      const updated = await updatePortalSettings(settings);
      setSettings(updated);
    ...
    <div className="min-h-screen bg-[#f4f3ef] text-neutral-800 font-sans flex flex-col antialiased">
      ...
      <header className="bg-[#1c140d] text-white py-4 px-6 md:px-12 flex items-center justify-between shadow-md shrink-0 border-b border-neutral-800">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2.5 px-3.5 py-1.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-md mr-2 select-none">
            <div className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span>
            </div>
            <div className="flex flex-col text-left leading-none">
              <span className="text-[9px] font-bold text-neutral-400 uppercase tracking-widest">Website Visitors</span>
              <span className="text-xs font-black text-white mt-0.5">
                {globalSettings?.totalVisits || 247} <span className="text-[9px] font-semibold text-green-400 ml-1">({Math.max(1, chatSessions.filter(s => !s.isClosed).length)} Online)</span>
              </span>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-[1500px] w-full mx-auto p-4 md:p-8 flex flex-col md:flex-row gap-6 items-start">
        ...
        <section className="flex-1 w-full min-w-0">
          <AnimatePresence mode="wait">
                ...
                <form onSubmit={handleSaveSettings} className="space-y-6 mt-8 text-left">
                  ...
                  <div className="border-t border-[#eae8e1] pt-6 grid sm:grid-cols-3 gap-4">
                    ...
                    <div className="flex items-center justify-between p-4 bg-red-50/20 rounded-xl border border-red-100/50">
                      <div>
                        <span className="text-xs font-bold text-red-800 block">Maintenance Mode</span>
                        <span className="text-[10px] text-red-500/80 font-light">Put frontend offline (admin remains live)</span>
                      </div>
                      <label className={`relative inline-flex items-center ${isViewer ? "cursor-not-allowed opacity-50" : "cursor-pointer"}`}>
                        <input
                          type="checkbox"
                          ...
                          className="sr-only peer"
                        />
                        <div className="w-9 h-5 bg-neutral-200 rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-red-500" />
                      </label>
                    </div>
                  </div>
                        ...
                        return (
                          <div key={session.id} className="relative group">
                            <button
                              ...
                              className={`w-full text-left p-4 pr-10 transition-all duration-200 cursor-pointer flex flex-col gap-1.5 ${activeSessionId === session.id
                                ? "bg-[#577a4c]/5 border-l-4 border-[#577a4c]"
                                : "hover:bg-neutral-50/80 bg-transparent border-l-4 border-transparent"
                                }`}
                            >
                              <div className="flex items-center justify-between">
                                <span className="text-xs font-bold text-neutral-900 block truncate max-w-[130px]">
                                  {session.clientName}
                                </span>
                            ...
                            <button
                              ...
                              className="absolute top-3 right-2.5 opacity-0 group-hover:opacity-100 transition-opacity duration-150 p-1 rounded-md text-neutral-400 hover:text-red-500 hover:bg-red-50 cursor-pointer"
                            >
                              <Trash2 className="h-3.5 w-3.5" />
                            </button>
                          </div>
            ...
            {activeTab === "gallery" && (() => {
              // Gather dynamic categories from the current list
              ...
              const filteredPhotos = galleryFilter === "all"
                ? galleryPhotos
                : galleryPhotos.filter(p => (p.category || "General") === galleryFilter);
              ...
              return (
                ...
                >
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {selectMode && filteredPhotos.length > 0 && (
                          <button
                            type="button"
                            onClick={() => {
                              if (selectedIndices.size === filteredPhotos.length) {
                                setSelectedIndices(new Set());
                              } else {
                                const idxs = new Set<number>();
                                filteredPhotos.forEach(p => {
                                  const origIdx = galleryPhotos.findIndex(x => x.id === p.id);
                                  if (origIdx !== -1) idxs.add(origIdx);
                                });
                          ...
                          >
                            {selectedIndices.size === filteredPhotos.length ? "Deselect All" : "Select All"}
                          </button>
                        )}
                  ...
                  {/* Upload Panel (only for editors/admins when not in multi-select mode) */}
                  {!isViewer && !selectMode && (
                    <div className="bg-[#fcfbf8] border border-[#eae8e1] rounded-2xl p-5 space-y-4">
                      <h3 className="text-xs font-bold text-neutral-800 uppercase tracking-wider font-serif">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                        ...
                        <div className="space-y-1">
                          <label className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider block">
                            2. Choose Project Image(s)
                          </label>
                          <label className="flex items-center justify-between bg-white border border-neutral-200 hover:border-[#577a4c] rounded-xl px-4 py-2.5 cursor-pointer text-xs text-neutral-600 transition-all select-none">
                            <span className="truncate max-w-[180px]">
                              {selectedUploadFiles.length === 0 
                                ? "Browse images..." 
                                : `${selectedUploadFiles.length} file(s) selected`}
                            </span>
                            <Plus className="h-4.5 w-4.5 text-[#577a4c]" />
                            <input
                              type="file"
                              accept="image/*"
                              multiple
                        ...
                        <button
                          type="button"
                          onClick={handleExecuteGalleryPhotoUpload}
                          disabled={selectedUploadFiles.length === 0}
                          className={`w-full py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all duration-200 shadow-md flex items-center justify-center gap-2 ${
                            selectedUploadFiles.length === 0
                              ? "bg-neutral-200 text-neutral-400 cursor-not-allowed shadow-none"
                              : "bg-[#577a4c] hover:bg-[#3d5636] text-white cursor-pointer"
                          }`}
                        >
                          Upload & Assign Category
                        </button>
                      </div>
                        ...
                        <div className="flex flex-wrap gap-2 pt-3 border-t border-neutral-100 items-center justify-between">
                          <div className="flex flex-wrap gap-1.5 max-h-[60px] overflow-y-auto">
                            {selectedUploadFiles.map((file, idx) => (
                              <span key={idx} className="inline-flex items-center gap-1 bg-white border border-neutral-200 text-[10px] text-neutral-600 px-2 py-0.5 rounded-md">
                          ...
                          <button
                            ...
                            className="text-red-600 hover:text-red-700 font-bold text-[10px] uppercase tracking-wider cursor-pointer"
                          >
                            Clear Queue
                          </button>
                        </div>
                      ...
                      {dynamicCategories.map((cat) => (
                        <button
                          ...
                          onClick={() => {
                            ...
                            setSelectedIndices(new Set()); // Reset selections on filter change
                          }}
                          className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all duration-200 border cursor-pointer ${
                            galleryFilter === cat
                              ? "bg-[#577a4c] text-white border-[#577a4c] shadow-xs"
                  ...
                  {filteredPhotos.length === 0 ? (
                    <div className="py-20 text-center text-neutral-400 font-light text-sm flex flex-col items-center justify-center gap-2 bg-neutral-50 rounded-2xl border border-neutral-100">
                      <ImageIcon className="h-10 w-10 text-neutral-300" />
                      <p>No photos in this category.</p>
                      <p className="text-xs text-neutral-400">Upload new project pictures or change category filters.</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {filteredPhotos.map((photo) => {
                        ...
                        return (
                          <div
                            key={photo.id}
                            className={`group relative aspect-video rounded-xl overflow-hidden border-2 bg-neutral-50 shadow-xs hover:shadow-md transition-all duration-300 cursor-pointer ${
                              isSelected ? "border-[#577a4c] ring-2 ring-[#577a4c]/40" : "border-neutral-200"
                            }`}
                            onClick={() => selectMode && originalIndex !== -1 ? toggleSelectPhoto(originalIndex) : undefined}
                          >
                            <img
                              src={photo.url}
                            ...
                            <div className="absolute top-2 left-2 bg-[#2d3f26]/90 border border-[#23321e]/50 backdrop-blur-xs text-white text-[9px] font-extrabold px-2 py-0.5 rounded-full uppercase tracking-wider shadow-sm select-none z-10">
                              {photo.category || "General"}
                            </div>

                            {/* Title Overlay in bottom-left */}
                            {photo.title && (
                              <div className="absolute bottom-2 left-2 right-2 bg-black/60 backdrop-blur-xs text-white text-[10px] font-bold px-2 py-1 rounded-lg truncate pointer-events-none z-10 border border-white/10 opacity-80 group-hover:opacity-100 transition-opacity">
                                {photo.title}
                              </div>
                            )}
                            ...
                            {selectMode && (
                              <div className={`absolute inset-0 transition-all duration-200 flex items-start justify-end p-2.5 z-20 ${isSelected ? "bg-[#577a4c]/20" : "bg-black/10 opacity-0 group-hover:opacity-100"
                                }`}>
                            ...
                            {!selectMode && (
                              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-2 z-20">
                                <button
                                  ...
                                  className="bg-white hover:bg-neutral-100 text-neutral-800 rounded-full px-2.5 py-1.5 shadow-md transition-all cursor-pointer text-[9px] font-extrabold uppercase tracking-wider border border-neutral-200"
                                  title="Preview Image"
                                >
                                  Zoom
                                </button>
                                {!isViewer && (
                                  <button
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleOpenEditPhotoDetails(photo);
                                    }}
                                    className="bg-[#577a4c] hover:bg-[#3d5636] text-white rounded-full px-2.5 py-1.5 shadow-md transition-all cursor-pointer text-[9px] font-extrabold uppercase tracking-wider"
                                    title="Edit details (title, category, caption)"
                                  >
                                    Edit
                                  </button>
                                )}
                                {!isViewer && originalIndex !== -1 && (
                                  <button
                                    type="button"
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleGalleryPhotoRemove(originalIndex);
                                    }}
                                    className="bg-red-600 hover:bg-red-700 text-white rounded-full p-2.5 shadow-md transition-all cursor-pointer"
                                    title="Delete Photo"
                                  >
                                    <Trash2 className="h-3.5 w-3.5" />
                                  </button>
                  ...
                  {editingPhoto && (
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-[999] flex items-center justify-center p-4">
                      <div className="bg-white border border-[#eae8e1] rounded-2xl p-6 shadow-2xl max-w-md w-full text-left space-y-4 animate-in zoom-in duration-200">
                        ...
                        <form onSubmit={handleSavePhotoDetails} className="space-y-4">
                          <div className="flex items-center justify-center rounded-lg border border-neutral-200 overflow-hidden bg-neutral-50 max-h-[160px]">
                            <img src={editingPhoto.url} alt="Preview" className="object-contain max-h-[160px]" />
                          </div>
                          ...
                          <div className="space-y-1">
                            ...
                            <input
                              type="text"
                              placeholder="e.g. Luxury Flagstone Patio"
                          ...
                          <div className="space-y-1">
                            ...
                            <textarea
                              placeholder="Provide a brief description of this work..."
                              rows={3}
                              value={editPhotoCaption}
                              onChange={(e) => setEditPhotoCaption(e.target.value)}
                              className="w-full bg-[#fcfbf8] border border-neutral-200 rounded-xl px-3.5 py-2 text-xs text-neutral-800 focus:outline-none focus:ring-1 focus:ring-[#577a4c] resize-none"
                            />
                          </div>
                          ...
                          <div className="flex justify-end gap-2 pt-3 border-t border-neutral-100">
                            <button
                              type="button"
                              onClick={() => setEditingPhoto(null)}
                              className="px-4 py-2 border border-neutral-200 rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-neutral-50 cursor-pointer"
                            >
                              Cancel
                            </button>
                            <button
                              type="submit"
                              className="px-4 py-2 bg-[#577a4c] hover:bg-[#3d5636] text-white rounded-xl text-xs font-bold uppercase tracking-wider shadow-md cursor-pointer"
                            >
                              Save Details
                            </button>
                          </div>
            ...
            {activeTab === "emails" && (
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-neutral-100 pb-5">
                  ...
                  <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-2.5 shrink-0">
                    <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                    <div>
                      <p className="text-[10px] font-bold text-emerald-700 uppercase tracking-wider">Auto-Forwarded</p>
                      <p className="text-[10px] text-emerald-600 font-medium">robert@jrmconstructionservicellc.com</p>
                    </div>
                  </div>
                </div>
            ...
            {activeTab === "security" && (
              ...
              >
                ...
                <div className="grid lg:grid-cols-12 gap-6 items-start">
                  ...
                  <div className={`bg-white border border-[#eae8e1] rounded-2xl p-6 shadow-xs text-left space-y-5 ${
                    currentUser?.role === "admin" ? "lg:col-span-5" : "col-span-12 max-w-md mx-auto w-full"
                  }`}>
                    <div>
                    ...
                    <form onSubmit={handleUpdateProfile} className="space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-[#fbfaf7] disabled:opacity-60 rounded-lg border border-neutral-200 px-3.5 py-2 text-xs text-neutral-800 focus:outline-none focus:ring-1 focus:ring-[#577a4c]"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder={isViewer ? "Read-only" : "Required to change password"}
                          className="w-full bg-[#fbfaf7] disabled:opacity-60 rounded-lg border border-neutral-200 px-3.5 py-2 text-xs text-neutral-800 focus:outline-none focus:ring-1 focus:ring-[#577a4c]"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder={isViewer ? "Read-only" : "••••••••"}
                          className="w-full bg-[#fbfaf7] disabled:opacity-60 rounded-lg border border-neutral-200 px-3.5 py-2 text-xs text-neutral-800 focus:outline-none focus:ring-1 focus:ring-[#577a4c]"
                        />
                      </div>
                      ...
                      <button
                        type="submit"
                        disabled={isViewer}
                        className="w-full bg-[#577a4c] disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#3d5636] text-white py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all shadow-md cursor-pointer select-none active:scale-98"
                      >
                        {isViewer ? "Read-Only Profile" : "Update Credentials"}
                      </button>
                      {isViewer && (
                        <p className="text-[9.5px] text-red-500 font-light mt-1 text-center">Viewer accounts are read-only.</p>
                      )}
                    </form>
                  </div>
                                  ...
                                  <div className="flex justify-end items-center gap-2">
                                    {!isSelf && (
                                      <button
                                        ...
                                        className="p-1.5 bg-neutral-100 hover:bg-neutral-200 text-neutral-650 rounded-lg transition-all cursor-pointer"
                                    ...
                                    {isPrimaryAdmin ? (
                                      <span className="text-[9.5px] text-neutral-400 font-light italic px-2">System Account</span>
                                    ) : isSelf ? (
                                      <span className="text-[9.5px] text-neutral-400 font-light italic px-2">Logged In</span>
                                    ) : (
                                      <button
                                        type="button"
                                        onClick={() => handleDeleteUser(user.id, user.username)}
                                        className="p-1.5 bg-red-50 hover:bg-red-100 text-red-650 rounded-lg transition-all cursor-pointer"
                                        title="Delete User"
                                      >
                                        <Trash2 className="h-3.5 w-3.5" />
                                      </button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs select-none">
            ...
            >
              ...
              <div className="p-6 space-y-4 flex-1 overflow-y-auto max-h-[70vh]">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] text-neutral-400 font-bold uppercase tracking-wider mb-1">Client Name</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-[#fbfaf7] rounded-lg border border-neutral-200 pl-9 pr-3 py-1.5 text-xs text-neutral-800 font-semibold focus:outline-none focus:ring-1 focus:ring-[#577a4c]"
                      />
                    </div>
                  </div>
                  <div>
                    <span className="text-[10px] text-neutral-400 font-bold uppercase tracking-wider block mb-1">Date Submitted</span>
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-neutral-100 bg-neutral-50/50 text-neutral-500 text-[11px] font-medium w-full">
                      <Calendar className="h-3.5 w-3.5 text-neutral-400" />
                      {new Date(selectedLead.createdAt).toLocaleString([], { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" })}
                    </span>
                  </div>
                  <div>
                    <label className="block text-[10px] text-neutral-400 font-bold uppercase tracking-wider mb-1">Phone Number</label>
                    <div className="relative">
                  ...
                  <div>
                    <label className="block text-[10px] text-neutral-400 font-bold uppercase tracking-wider mb-1">Email Address</label>
                    <div className="relative">
                ...
                <div className="border-t border-neutral-100 pt-4 grid sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    <label className="block text-[10px] text-neutral-400 font-bold uppercase tracking-wider mb-1">Property Address</label>
                    <div className="relative">
                ...
                <div className="border-t border-neutral-100 pt-4 grid sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    <label className="block text-[10px] text-neutral-400 font-bold uppercase tracking-wider mb-1">Project Type</label>
                    <select
                      disabled={isViewer}
                      value={editProjectType}
                      onChange={(e) => setEditProjectType(e.target.value)}
                      className="w-full bg-[#fbfaf7] rounded-lg border border-neutral-200 px-3 py-1.5 text-xs text-neutral-800 font-semibold focus:outline-none focus:ring-1 focus:ring-[#577a4c] cursor-pointer"
                    >
                      <option value="remodeling">House Remodeling</option>
                      <option value="new-construction">New Construction</option>
                      <option value="outdoor-kitchen">Outdoor Kitchen</option>
                ...
                <div className="border-t border-neutral-100 pt-4">
                  <label className="block text-[10px] text-neutral-400 font-bold uppercase tracking-wider mb-1">Project Description</label>
                  <textarea
                    disabled={isViewer}
                    rows={3}
                    value={editDescription}
                    onChange={(e) => setEditDescription(e.target.value)}
                    className="w-full bg-[#fbfaf7] rounded-lg border border-neutral-200 p-3 text-xs text-neutral-800 font-medium focus:outline-none focus:ring-1 focus:ring-[#577a4c] leading-relaxed resize-none"
                  />
                </div>
        ...
        {resettingUser && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs select-none">
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-white border border-[#eae8e1] rounded-2xl shadow-xl w-full max-w-sm overflow-hidden"
            >
              <div className="p-5 border-b border-[#eae8e1] bg-neutral-50 flex items-center justify-between">
                <div className="text-left">
                  <h3 className="text-sm font-bold text-neutral-900 uppercase tracking-wide">Reset User Password</h3>
                  <p className="text-[10px] text-neutral-400 font-light mt-0.5">Change password for account: <strong className="text-neutral-700 capitalize">{resettingUser.username}</strong></p>
                </div>
                <button
                  onClick={() => { setResettingUser(null); setResetNewPassword(""); }}
                  className="text-neutral-400 hover:text-neutral-650 text-lg font-light transition-colors cursor-pointer"
                >
                  &times;
                </button>
              </div>

              <form onSubmit={handleAdminResetPassword} className="p-5 space-y-4 text-left">
                <div>
                  <label className="block text-[10px] font-bold text-neutral-500 uppercase tracking-wider mb-1">
                ...
                <div className="flex items-center justify-end gap-3 pt-2">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 bg-[#577a4c] hover:bg-[#3d5636] text-white rounded-xl font-bold text-xs uppercase tracking-wider transition-all duration-200 shadow-md cursor-pointer select-none active:scale-98"
                  >
                    Save Password
                  </button>
                </div>
